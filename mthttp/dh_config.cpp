...
struct dh_http_settings settings;
...
static int read_config_from_json(json_t *root)
{
    ...
    // ret = load_cfg_rpc_clt(root, "main", &settings.main);
    // if (ret < 0) {
    //     printf("load main clt config fail: %d\n", ret);
    //     return -__LINE__;
    // }
    ...
    //     return -__LINE__;
    // }
    /*ret = load_cfg_rpc_clt(root, "read_mareting_sort", &settings.read_mareting_sort);
    if (ret < 0) {
        printf("load readhistory clt config fail: %d\n", ret);
        return -__LINE__;
    }*/

    ret = load_cfg_rpc_clt(root, "mt", &settings.mt);
    if (ret < 0) {
        printf("load readhistory clt config fail: %d\n", ret);
        return -__LINE__;
    }
...
int init_dice_config(const char *path)
{
    json_error_t error;
