...
int main(int argc, char *argv[]) {
	...
	fl_add_block_callback(handle_mt_block);

	ret = init_fullnode();
	if (ret < 0) {
		error(EXIT_FAILURE, errno, "init init_fullnode fail: %d", ret);
	}

	ret = init_history();
	if (ret < 0) {
		error(EXIT_FAILURE, errno, "init init_history fail: %d", ret);
	}

	/*ret = init_mt_redeme_query();
	  if (ret < 0) {
	  error(EXIT_FAILURE, errno, "init init_mt_redeme_query fail: %d", ret);
	  }*/
