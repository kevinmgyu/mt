...
static int read_config_from_json(json_t *root) {
	...
	ret = read_cfg_int(root, "flush_mysql_interval", &mt_settings.flush_num, false, 10);
	if (ret < 0) {
		printf("load flush_mysql_interval fail: %d", ret);
		return -__LINE__;
	}
...
int init_mt_config(const char *path) {
	...
	if (!json_is_object(root)) {
	  ...
	  ret = read_config_from_json(root);
	if (ret < 0) {
		json_decref(root);
		return ret;
	}
